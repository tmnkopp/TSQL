...
DECLARE @TNAM  AS NVARCHAR(55)='fsma_Milestones'; 
SELECT @QUOTENAME = @QUOTENAME + QUOTENAME(COLUMN_NAME) + ',' FROM (SELECT COLUMN_NAME, DATA_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME=@TNAM AND COLUMN_NAME NOT LIKE '%PK_%' AND COLUMN_NAME <> 'IsActive'  AND COLUMN_NAME <> 'DateCreated' AND COLUMN_NAME <> 'DateModified' AND COLUMN_NAME <> 'DateUpdated') vw
...
SELECT TOP 0 PK_PrimeKey, Change_date, [Status],[AssignedTo],[Title],[TasksDesc],[ResourceReqmts],[MitConstraints],[ConExplanation],[SchCompDate],[ActCompDate],[Comments] FROM ( 
	SELECT Change_date, FieldValue, FieldName, PK_PrimeKey FROM ( 
		 SELECT FLATBYDATE.PK_AuditLog, Change_date, PK_PrimeKey, FieldName, FieldValue FROM ( 
			SELECT MAX(PK_AuditLog) PK_AuditLog, Change_date, PK_PrimeKey, FieldName FROM ( 
				SELECT  CONVERT(NVARCHAR(11), Change_date, 121)  Change_date,  PK_AuditLog , PK_PrimeKey, FieldName FROM AuditLog    
			) ALOG GROUP BY Change_date,  PK_PrimeKey, FieldName  
		 ) FLATBYDATE
		 INNER JOIN (
...
SET @exe = @exe + ' 				SELECT  CONVERT(NVARCHAR(11), Change_date, 121)  Change_date,  PK_AuditLog , PK_PrimeKey, FieldName FROM AuditLog     '
