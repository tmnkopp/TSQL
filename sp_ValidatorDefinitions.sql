...
Who:	T.KOPP
When:	9-5-2020
Why:	Provides validation definitions (regex formulas, error messages) - use validation code for in-code references
How:	
...
BEGIN 
	...
	BEGIN
		...
		INSERT INTO @VALIDATIONS(ValidationCode, ValidationType, Expression, ErrorMessage ) VALUES  
			('Require' ,'FUNC','','{0} is a required field.')
		,   ('MaxLength' ,'FUNC','','{0} must be fewer than {1} characters.')
		,   ('MinLength'  ,'FUNC','','{0} must be at least {1} characters.')
		,   ('Equals' ,'FUNC','','{0} must equal {1}.')
		,   ('DoesNotEqual' ,'FUNC','','{0} cannot equal {1}.')
		,   ('DoesNotEqualAny' ,'FUNC','','{0} cannot equal any of the following ({1}).')
		,   ('EqualsAny' ,'FUNC','','{0} must equal any of the following ({1}).')
		,	('IPADDRESS', 'REGEX'
