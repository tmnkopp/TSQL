DECLARE @PK_FORM_TO NVARCHAR(15) = '2020-Q4-CIO'  
DECLARE @AG_ACRONYM NVARCHAR(5) = 'DOJ'   
DECLARE @PK_FORM_TYPE NVARCHAR(15) = SUBSTRING( @PK_FORM_TO , 5, LEN(@PK_FORM_TO))  
 
--SELECT * FROM vw_MetricsAnswers WHERE PK_Form='2017-Q4-CIO'   
--INSERT INTO fsma_Answers (Answer, FK_Question, FK_OrgSubmission ,DateModifed, Answer2 ) 

SELECT MAX(ISNULL(Answer,'0')), QPKTO, ORG_TO.PK_OrgSubmission, GETDATE(), '[::]'-- USE TO IDINTIFY THESE RECORDS TO DELETE 
FROM  vw_MetricsAnswers
LEFT JOIN (
	SELECT DISTINCT ID IDTO, QPK QPKTO FROM vw_MetricsAnswers WHERE PK_FORM = @PK_FORM_TO
...
WHERE ( PK_FORM LIKE '%'+@PK_FORM_TYPE ) AND QPKTO IS NOT NULL
