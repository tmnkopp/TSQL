/*
	...
	EXEC cq_HVAInSubmissionHistory @PK_ReportingCycle='88', @PK_Component='170', @HVASubmissionDate ='2020-07-23'
*/ 
IF EXISTS (SELECT * FROM sysobjects WHERE name = 'cq_HVAInSubmissionHistory' AND type = 'P')
    DROP PROCEDURE cq_HVAInSubmissionHistory
GO 
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO 
CREATE PROCEDURE [cq_HVAInSubmissionHistory]
    @UserID smallint = 0,
...
--BEGIN QUERY
	...
	SELECT * INTO #AUDITS_MASTER FROM (
		...
		WHERE #AUDITVIEW.TableName='HVAIn' 
	)	HVAIn_Audits 
	...
	SELECT * INTO #SUBMISSION_VIEW FROM (
		...
		, CASE 
			...
			WHEN #FIELD_META.FieldName  IN ('IsInternal','IsHVA','IsEssential' ) THEN 
				CASE 
	...
	SET @exe=@exe+'  SELECT HVA, SystemName, IsInternal, IsHVA, IsEssential FROM ('
...
GO 
EXEC cq_DeleteQueryAndClearRefs 'cq_HVAInSubmissionHistory'  
EXEC cq_CreateOrReplaceQueryAndClearRefs 'cq_HVAInSubmissionHistory', 'BOD 18-02 Data - Agency HVA Internal Interconnection Submissions', 12, 0 
EXEC cq_CreateOrUpdateSPtoParms 'cq_HVAInSubmissionHistory','orgsubmission',NULL,NULL,1,1,0,0,0,NULL,NULL,'bod1802' 
EXEC cq_CreateOrUpdateSPtoParms 'cq_HVAInSubmissionHistory','UserID',NULL,NULL,2  
EXEC cq_CreateOrUpdateSPtoParms 'cq_HVAInSubmissionHistory','HVASubmissionDate',NULL,'Submission Date',3
EXEC cq_CreateOrUpdateSPtoPermissions 'cq_HVAInSubmissionHistory', 'OMBHVAPOAMDC' 
EXEC cq_MasterSort 
GO 